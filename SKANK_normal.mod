...
// Normal model: macroprudencial rule binds
...
// Fourth: Pure forward-looking variables (6)
    X1 X2 piee Lambda RN rK;

// Alert RegGae: variables in all regime-.mod files must be declared in the same order
...
//endogenous 
bR_ss D_ss R_ss F_ss Y_ss; // the variable parameter should be declared last
...
RD*(1-zetaD)=RF*(1-zetaF); 
RT=RD*(1-zetaD); 
bI=RN*N(-1)/(wD*(1-zetaD)*RD(-1)*D(-1)+wF*(1-zetaF)*RF(-1)*F(-1)); 
b=(bR+vB+bI)/2; //Normal regime
1+bR=(1+bR_ss)*(((1+bR_ss)/(1+bR(-1)))^rho_bR)*(((D+F)/(D_ss+F_ss))^phi_DY)*exp(siggma_bR*(eps_bR));
OmegaD=zetaD*RD(-1)/piee*D(-1);
...
steady;
check;
//stoch_simul(order=1, noprint, irf=10); // , 
